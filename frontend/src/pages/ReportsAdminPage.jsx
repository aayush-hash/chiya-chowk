  ...
  useEffect(() => {
    ...
    if (lineRef.current) {
      ...
      lineChart.current = new Chart(lineRef.current, {
        ...
        data: {
          ...
          datasets: [{ label: 'Revenue', data: days.map(d => d.revenue), borderColor: '#d4862a', backgroundColor: 'rgba(212,134,42,0.1)', fill: true, tension: 0.4, pointRadius: 4, pointBackgroundColor: '#d4862a' }],
        },
        options: { responsive: true, plugins: { legend: { display: false } }, scales: { x: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#7a6a5a', font: { size: 10 } } }, y: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#7a6a5a', font: { size: 10 } } } } },
      });
    }
    if (doughnutRef.current && report.categoryStats?.length) {
      ...
      doughnutChart.current = new Chart(doughnutRef.current, {
        type: 'doughnut',
        data: { labels: cs.map(c => c._id), datasets: [{ data: cs.map(c => c.revenue), backgroundColor: ['rgba(212,134,42,0.7)', 'rgba(76,175,136,0.7)', 'rgba(91,155,213,0.7)', 'rgba(155,126,212,0.7)', 'rgba(224,92,92,0.7)', 'rgba(255,200,100,0.7)'], borderWidth: 2 }] },
        options: { responsive: true, plugins: { legend: { position: 'bottom', labels: { color: '#b8a898', font: { size: 11 } } } }, cutout: '60%' },
      });
    }
  }, [report]);

  useEffect(() => () => { lineChart.current?.destroy(); doughnutChart.current?.destroy(); }, []);
      ...
      {report && (
        <>
          ...
          <div className="grid-2 mb-24">
            <div className="card"><div className="card-title mb-16">Revenue Trend</div><canvas ref={lineRef} style={{ maxHeight: 220 }} /></div>
            <div className="card"><div className="card-title mb-16">Category Breakdown</div><canvas ref={doughnutRef} style={{ maxHeight: 220 }} /></div>
          </div>
          <div className="card">
            <div className="card-title mb-16">Daily Breakdown</div>
            <table className="data-table">
              <thead><tr><th>Date</th><th>Orders</th><th>Revenue</th><th>Cash</th><th>QR</th><th>Avg</th></tr></thead>
              <tbody>
                {[...report.dailyStats].reverse().map(d => (
                  <tr key={d._id}>
                    ...
                    <td style={{ fontFamily: 'DM Mono' }}>Rs. {d.orders ? Math.round((d.revenue || 0) / d.orders).toLocaleString() : 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card">
          ...
          <form onSubmit={addUser}>
            <div className="grid-2" style={{ gap: 8 }}>
              <input className="form-control" placeholder="Full name" value={newUser.name} onChange={e => setNewUser(f => ({ ...f, name: e.target.value }))} required />
              <input className="form-control" placeholder="Username" value={newUser.username} onChange={e => setNewUser(f => ({ ...f, username: e.target.value }))} required />
            </div>
            <div className="grid-2" style={{ gap: 8, marginTop: 8 }}>
              <input type="password" className="form-control" placeholder="Password" value={newUser.password} onChange={e => setNewUser(f => ({ ...f, password: e.target.value }))} required />
            ...
            <button type="submit" className="btn btn-primary btn-full" style={{ marginTop: 10 }}>+ Add Member</button>
          </form>
        </div>
            ...
            <>
              {[
                ...
                <div key={f.key} className="form-group">
                  ...
                  <input type={f.type} className="form-control" value={settings[f.key] || ''} onChange={e => setSettings(s => ({ ...s, [f.key]: f.type === 'number' ? parseFloat(e.target.value) : e.target.value }))} />
                </div>
              ))}
              <div className="form-group">
                <label className="form-label">Receipt Footer Message</label>
                <textarea className="form-control" rows={2} value={settings.receiptFooter || ''} onChange={e => setSettings(s => ({ ...s, receiptFooter: e.target.value }))} style={{ resize: 'none' }} />
              </div>
...
export const TransactionsPage = () => {
  ...
  const exportCSV = () => {
    const headers = ['Order ID', 'Table', 'Items', 'Subtotal', 'Tax', 'Total', 'Payment', 'Status', 'Time', 'Cashier'];
    const rows = orders.map(o => [o.orderId, o.tableNumber || o.orderType, o.items.map(i => `${i.name}x${i.qty}`).join(';'), o.subtotal, o.taxAmount, o.total, o.paymentMethod, o.paymentStatus, new Date(o.createdAt).toLocaleString(), o.cashierName]);
    const csv = [headers, ...rows].map(r => r.join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = `txn-${dateFilter}.csv`; a.click();
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="stats-grid mb-24" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        <div className="stat-card"><div className="stat-label">Collected</div><div className="stat-value mono" style={{ fontSize: 20 }}>Rs. {totalRev.toLocaleString()}</div></div>
        <div className="stat-card green"><div className="stat-label">Cash</div><div className="stat-value mono" style={{ fontSize: 20 }}>Rs. {cashRev.toLocaleString()}</div></div>
        <div className="stat-card blue"><div className="stat-label">QR/eSewa</div><div className="stat-value mono" style={{ fontSize: 20 }}>Rs. {qrRev.toLocaleString()}</div></div>
        <div className="stat-card red"><div className="stat-label">Unpaid</div><div className="stat-value mono" style={{ fontSize: 20 }}>Rs. {unpaid.toLocaleString()}</div></div>
      </div>
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
        {loading ? <div className="flex-center" style={{ padding: 40 }}><div className="spinner" /></div> : (
          <table className="data-table">
            <thead><tr><th>Order ID</th><th>Table</th><th>Amount</th><th>Tax</th><th>Method</th><th>Status</th><th>Time</th><th>Cashier</th></tr></thead>
            <tbody>
              {orders.length === 0 ? (
                <tr><td colSpan={8} style={{ textAlign: 'center', color: 'var(--text3)', padding: 30 }}>No transactions</td></tr>
              ) : orders.map(o => (
                <tr key={o._id}>
                  ...
                  <td><span style={{ fontFamily: 'DM Mono', fontSize: 12 }}>Rs. {o.taxAmount.toLocaleString()}</span></td>
